...
export default function App() {
  ...
  const handleClaroClick = async () => {
    ...
    setMessages(prev => [...prev, { id: 'f_end', type: 'received', text: 'Qual você vai preferir que eu te mostre primeiro amor? 🔥', time: getTime() }]);
  ...
  const handleOptionClick = async (choice: string) => {
    ...
    setIsTyping(false);
    setMessages(prev => [...prev, { id: generateId(), type: 'received', text: 'To sozinha agora e louca pra me divertir com você amor 😈', time: getTime() }]);
    ...
    await wait(2800);
    setIsTyping(false);
    setMessages(prev => [...prev, { id: generateId(), type: 'received', text: 'Vamos fazer uma chamadinha de video comigo ou prefere o Pacote de fotos e videos ?', time: getTime() }]);
  ...
  const handleUpsellClick = async (choiceType: 'video' | 'pacote') => {
    ...
    if (choiceType === 'video') {
      ...
      await wait(2000);
      setIsTyping(false);
      setMessages(prev => [...prev, { id: generateId(), type: 'received', text: 'Vai querer quanto tempo hoje pra gente curtir e brincar gostoso?', time: getTime() }]);
      ...
      await wait(2500);
      setIsTyping(false);
      setMessages(prev => [...prev, { id: generateId(), type: 'received', text: 'Amor os conteudos são todos longos, mostro cada detalhezinho bem de perto pra você', time: getTime() }]);

      await wait(1500);
      setIsTyping(true);
      await wait(2000);
      setIsTyping(false);
      setMessages(prev => [...prev, { id: generateId(), type: 'received', text: 'Qual pacote você vai querer hoje pra ver tudo bem rosinha e sem frescura?', time: getTime() }]);
      ...
      <div className="w-full h-[100dvh] sm:h-[720px] sm:max-h-none sm:max-w-[360px] bg-[#e5ddd5] sm:rounded-[20px] overflow-hidden flex flex-col sm:shadow-[0_10px_25px_rgba(0,0,0,0.2)] relative">
        ...
        >
          ...
          {showVideoCallPricing && (
            <div className="flex w-full justify-end mb-4 animate-fade-in-up md:pr-2">
              <div className="flex w-full max-w-[95%] gap-[12px] justify-end flex-wrap">
                <button onClick={() => window.location.href = 'https://compraseguraonline.org.ua/c/04ae4bf681'} className="bg-[#015c4b] text-white font-bold text-[13px] py-[10px] px-[12px] rounded-[6px] shadow-sm transition-transform hover:scale-105 active:scale-95 relative tracking-wide cursor-pointer flex-1 min-w-[80px] text-center">
                  3 Minutos
                  <span className="absolute -top-[6px] -right-[6px] w-[14px] h-[14px] rounded-full bg-[#00a884]"></span>
                </button>
                <button onClick={() => window.location.href = 'https://compraseguraonline.org.ua/c/e1d2848384'} className="bg-[#015c4b] text-white font-bold text-[13px] py-[10px] px-[12px] rounded-[6px] shadow-sm transition-transform hover:scale-105 active:scale-95 relative tracking-wide cursor-pointer flex-1 min-w-[80px] text-center">
                  5 Minutos
                  <span className="absolute -top-[6px] -right-[6px] w-[14px] h-[14px] rounded-full bg-[#00a884]"></span>
                </button>
                <button onClick={() => window.location.href = 'https://compraseguraonline.org.ua/c/15925e9636'} className="bg-[#015c4b] text-white font-bold text-[13px] py-[10px] px-[12px] rounded-[6px] shadow-sm transition-transform hover:scale-105 active:scale-95 relative tracking-wide cursor-pointer flex-1 min-w-[80px] text-center">
                  Até o limite
                  <span className="absolute -top-[6px] -right-[6px] w-[14px] h-[14px] rounded-full bg-[#00a884]"></span>
                </button>
              </div>
          ...
          {showPackagePricing && (
             <div className="flex w-full justify-end mb-4 animate-fade-in-up md:pr-2">
              <div className="flex w-full max-w-[95%] gap-[12px] justify-end flex-wrap">
                <button onClick={() => window.location.href = 'https://compraseguraonline.org.ua/c/10c726aed3'} className="bg-[#015c4b] text-white font-bold text-[13px] py-[10px] px-[12px] rounded-[6px] shadow-sm transition-transform hover:scale-105 active:scale-95 relative tracking-wide cursor-pointer flex-1 min-w-[70px] text-center">
                  Básico
                  <span className="absolute -top-[6px] -right-[6px] w-[14px] h-[14px] rounded-full bg-[#00a884]"></span>
                </button>
                <button onClick={() => window.location.href = 'https://compraseguraonline.org.ua/c/ea3a49b3d7'} className="bg-[#015c4b] text-white font-bold text-[13px] py-[10px] px-[12px] rounded-[6px] shadow-sm transition-transform hover:scale-105 active:scale-95 relative tracking-wide cursor-pointer flex-1 min-w-[70px] text-center">
                  Médio
                  <span className="absolute -top-[6px] -right-[6px] w-[14px] h-[14px] rounded-full bg-[#00a884]"></span>
                </button>
                <button onClick={() => window.location.href = 'https://compraseguraonline.org.ua/c/1f13c2044c'} className="bg-[#015c4b] text-white font-bold text-[13px] py-[10px] px-[12px] rounded-[6px] shadow-sm transition-transform hover:scale-105 active:scale-95 relative tracking-wide cursor-pointer flex-1 min-w-[70px] text-center">
                  Completo
                  <span className="absolute -top-[6px] -right-[6px] w-[14px] h-[14px] rounded-full bg-[#00a884]"></span>
                </button>
              </div>
